...
The `info@kgolaentle` access is either fully removed, or downgraded to a backup-only role (for example, kept as a secondary admin in case of emergency, but stripped of finance and ownership rights). The decision on which option to use depends on whether `info@` will continue to be monitored by the team.

Business contact details on the Portfolio reflect Kgolaentle Holdings' real registered information: company name, registration number 2014/077326/07, business address, and Lebogang as point of contact.
...
15. Update the Business Info on the Portfolio: registered legal name (Kgolaentle Holdings), reg number, address, and primary business contact (Lebogang).
16. Set Lebogang's email and phone as the recovery contacts where Meta allows it.
...
18. Log in as `lebogang@kgolaentle.com` and confirm you can: post on the Page, reply to a Page message, view ad account billing, send a WhatsApp Business test message, edit Instagram from the Portfolio, and view the Pixel.
19. Have Lebogang send a test post or a test message and confirm it goes out properly.
...
21. Decide: remove `info@kgolaentle` entirely, or downgrade it to a limited backup role.
...
- A real-world test (post + message reply + ad account view) was completed successfully from Lebogang's account.
- `info@kgolaentle` is either removed or explicitly documented as a downgraded backup role.
...
**The Page is owned by a Business Portfolio that cannot be transferred directly.** Meta does not allow Page ownership to move freely between Portfolios. If the current Portfolio is the wrong one, you may need to request the Page back into a new Portfolio you create under Lebogang. This often requires a Meta support ticket. Don't try to brute-force it.

**`info@kgolaentle` is the only admin, and no one can log in.** If access to that mailbox is shaky, do NOT remove it from any role until Lebogang's account is fully tested. If we're already locked out of `info@`, we may need to use Meta's account recovery process (ID upload, business documents) — slow but workable.

**Lebogang's email has no existing Facebook identity.** Meta admin roles attach to a Facebook profile, not an email address. A profile must exist first. Use the real name (it makes business verification easier later).
...
**Business verification is required.** Meta sometimes blocks ownership changes until the Portfolio is verified. Have CIPC documents (Reg 2014/077326/07), proof of address, and a business utility/bank document ready in PDF.
...
**Personal-account risk.** Lebogang's personal Facebook profile becomes a critical business asset the moment it holds these admin roles. It needs strong 2FA, a unique password, and ideally a backup admin person added to the Portfolio so we're never single-threaded on one human.
...
>
> **Final access model**
> Primary admin / owner: `lebogang@kgolaentle.com` (Full Control + Business Admin)
...
>
> **What remains**
> `info@kgolaentle` status: [Removed entirely / Downgraded to backup role] — circle one.
...
>
> - Login alerts on Lebogang's Facebook profile (any unexpected sign-in attempts)
...
> - Recovery email inbox (`lebogang@kgolaentle.com`) — make sure Meta's security emails are arriving and read
>
> **Notes for future**
