...
export default async function OpsIntelligencePage({
  ...
  const [recommendations, pendingByAgent, pendingDraftCount, draftsAwaiting, latestBriefing] = await Promise.all([
    ...
    db.opsDraftMessage.findMany({
      ...
      include: { recommendation: { select: { agentKey: true, entityType: true, entityId: true, summary: true } } },
      orderBy: { createdAt: 'desc' },
      take: 50,
    }),
    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Provider applications, profiles, request friction, and matching alerts.</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>WhatsApp drafts proposed by agents. Approving queues a draft — it does not send.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {draftsAwaiting.length === 0 && (
            <p className="text-muted-foreground text-sm">No drafts awaiting approval.</p>
          )}
          {draftsAwaiting.map((d) => (
            <div key={d.id} className="rounded-lg border p-4">
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="secondary">{AGENT_LABEL[d.recommendation.agentKey]}</Badge>
