...
const probes: Probe[] = [
  ...
  { migration: '20260429143000_wallet_status_ledger_entries', check: 'wallet_ledger_entries.status column exists', sql: `SELECT (COUNT(*) > 0)::int AS hit FROM information_schema.columns WHERE table_schema='public' AND table_name='wallet_ledger_entries' AND column_name='status'` },
  ...
  { migration: '20260430213000_wallet_ledger_status_and_adjustment_amounts', check: 'wallet_ledger_entries.adjustmentAmountCents column exists', sql: `SELECT (COUNT(*) > 0)::int AS hit FROM information_schema.columns WHERE table_schema='public' AND table_name='wallet_ledger_entries' AND column_name='adjustmentAmountCents'` },
