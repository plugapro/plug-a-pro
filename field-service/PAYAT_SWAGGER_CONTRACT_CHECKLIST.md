...
| Scope strings | Integrator/merchant RTP create: `rtp:create:single` | PayAtGo reads `PAYAT_GO_SCOPES`; legacy Pay@ token request now sends `PAYAT_SCOPES` or defaults to `rtp:create:single` | matches | Production disproof test proved unscoped token fails with 403 and scoped token succeeds. |
