-- Force reapply runtime schema after production DB restore.
--
-- WHY: On 2026-06-06, a production DB restore left the runtime database with
-- a `_prisma_migrations` ledger that marks today's drift-repair migrations as
-- applied while their DDL is missing from the live schema. `prisma migrate
-- deploy` then no-ops (it skips migrations already in the ledger), so the
-- runtime keeps throwing P2022/P2021-shaped Prisma errors against tables and
-- columns the code expects. Vercel runtime logs show this on:
--   /api/cron/provider-onboarding-recovery, /api/cron/expire-payment-intents,
--   /api/cron/session-timeout, /api/cron/match-leads,
--   /api/internal/cron/rebuild-candidate-pool,
--   /api/provider/registration/draft, POST /admin/applications
-- starting ~13:00 SAST while the migrate-deploy GH workflow at 15:39 SAST
-- still reports "Database schema is up to date!"
--
-- WHAT: This migration replays every today-dated drift-repair migration as
-- a fresh ledger entry. Every statement is wrapped in IF NOT EXISTS / DO
-- duplicate-object guards, so applying against a healthy DB is a no-op and
-- applying against a drifted DB fills the gaps.
--
-- ORIGINAL MIGRATIONS REPLAYED (in original order):
--   20260606090000_add_provider_resume_token
--   20260606115900_repair_production_schema_drift
--   20260606115930_enable_rls_after_schema_drift_repair
--   20260606120000_add_dispatch_decision_no_match_reason
--   20260606125000_enable_rls_repaired_matching_tables
--   20260606130000_provider_registration_drafts            (rewritten idempotent)
--   20260606145000_enable_provider_registration_rls
--
-- SAFETY: Additive only. No DROP, no RENAME, no data rewrites. Composite
-- replays of the existing migrations (which already proved safe earlier
-- today). Provider registration drafts replay was non-idempotent in its
-- original form; the rewrite below preserves the exact same target shape
-- using IF NOT EXISTS / EXCEPTION-tolerant constraint adds.


-- ─────────────────────────────────────────────────────────────────────────
-- Replay of 20260606090000_add_provider_resume_token
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- Replay of 20260606115900_repair_production_schema_drift
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- Replay of 20260606115930_enable_rls_after_schema_drift_repair
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- Replay of 20260606120000_add_dispatch_decision_no_match_reason
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- Replay of 20260606125000_enable_rls_repaired_matching_tables
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- Replay of 20260606130000_provider_registration_drafts (rewritten idempotent)
-- ─────────────────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS "provider_application_drafts" (
  "id" TEXT NOT NULL,
  "phone" TEXT NOT NULL,
  "email" TEXT,
  "name" TEXT,
...
CREATE TABLE IF NOT EXISTS "registration_resume_tokens" (
  "id" TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS "provider_application_drafts_submittedApplicationId_key" ON "provider_application_drafts"("submittedApplicationId");
CREATE INDEX IF NOT EXISTS "provider_application_drafts_phone_idx" ON "provider_application_drafts"("phone");
CREATE INDEX IF NOT EXISTS "provider_application_drafts_submittedApplicationId_idx" ON "provider_application_drafts"("submittedApplicationId");

CREATE UNIQUE INDEX IF NOT EXISTS "registration_resume_tokens_tokenHash_key" ON "registration_resume_tokens"("tokenHash");
CREATE INDEX IF NOT EXISTS "registration_resume_tokens_tokenHash_idx" ON "registration_resume_tokens"("tokenHash");
CREATE INDEX IF NOT EXISTS "registration_resume_tokens_draftId_idx" ON "registration_resume_tokens"("draftId");
CREATE INDEX IF NOT EXISTS "registration_resume_tokens_applicationId_idx" ON "registration_resume_tokens"("applicationId");
...
-- ─────────────────────────────────────────────────────────────────────────
-- Replay of 20260606145000_enable_provider_registration_rls
-- ─────────────────────────────────────────────────────────────────────────
