...
const { mockDb, mockNotifyExpiredJobParties } = vi.hoisted(() => ({
  mockDb: {
    ...
    // expireOpenJobRequest always runs the BOARD-lead close-out step after a
    // successful transition (main, provider lead board #192) — unrelated to
    // CJ-08's mid-funnel statuses, but the shared helper always executes it.
    lead: {
      findMany: vi.fn(),
      updateMany: vi.fn(),
    },
