...
- `SecurityEventType`: `OTP_REPORTED_UNREQUESTED, OTP_RATE_LIMIT_EXCEEDED, OTP_VERIFICATION_FAILED_REPEATEDLY, ACCOUNT_LOCKED, STEP_UP_COMPLETED, LOCK_CLEARED_BY_ADMIN`.
...
- attributes: `HttpOnly; SameSite=Lax; Path=/; Max-Age=600`.
...
- encryption: JSON payload encrypted with AES-256-GCM. Derive the 32-byte key
  from `OTP_HASH_PEPPER` using HKDF-SHA256 with salt
  `pap-step-up-token:v1` and info `plug-a-pro/security-step-up`.
...
| Point | File | Change |
|---|---|---|
| Code chokepoint | `app/api/auth/hooks/send-sms/route.ts` | If `!isDeliveryAllowed(phone)` → skip `deliverOtp`, record `CANCELLED` challenge + audit, return hook-success shape (no leak). Else `recordOtpChallenge()` → `deliverOtp()` → `markChallengeSent()` / `markChallengeSendFailed()`. |
...
| Inbound button | `app/api/webhooks/whatsapp/route.ts` → `lib/whatsapp-bot.ts` | Detect `otp_report_*` quick-reply payload → `reportUnrequestedOtp({ sourceChannel: WHATSAPP_BUTTON })`; idempotent (existing `InboundWhatsAppMessage` dedupe + service idempotency) |
