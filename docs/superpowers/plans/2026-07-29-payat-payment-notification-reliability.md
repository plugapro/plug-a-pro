...
> **Status 2026-07-30: implemented on `fix/payat-notification-reliability` (6 commits + a
> final-review fix wave, `6cf374f4..16b2033f`). Both flags ship OFF. The hazards below were
> discovered during implementation and review — read them before touching production.**

### Hazards found during implementation — read first

**H1. `PAYAT_SCOPES` is a live deployment hazard, not just a config step.**
`lib/payat/token.ts` mints **one** token for both create and read, scoped from `PAYAT_SCOPES`
(default `rtp:create:single`). Granting `rtp:read` in the Pay@ merchant portal is **not
sufficient** — the Vercel env var must change too. And per this repo's own
`PAYAT_SWAGGER_CONTRACT_CHECKLIST.md:45`, Pay@ enforces scopes at the **token** endpoint, so
widening `PAYAT_SCOPES` *before* the grant is live can get the token request itself rejected —
which breaks RTP **creation** for every provider, flags off included.
Sequence strictly: **grant in the portal → verify → then change the env var.**

**H2. The migration is a hard pre-deploy gate.**
`build` is `prisma generate && next build` — there is **no** `migrate deploy`. The
`clientAccountNumber` write sits inside a try/catch that swallows failures, so if the code
ships ahead of the migration, every Pay@ top-up in that window silently loses its lookup key
*and* its `sourceReference`/`paymentLink` metadata — manufacturing exactly the unreconcilable
rows this branch exists to eliminate. Run the migration first, confirm, then deploy.

**H3. Enable the sweep before the webhook, not the other way round.**
`payments.payat.readback_verification` enabled *alone* is strictly worse than today: if the
webhook fires while Pay@'s read view still shows the payment as outstanding, we ack it and
rely entirely on the sweep to catch up. Treat sweep-before-webhook as a hard dependency.

**H4. Nobody has seen a real `rtp/read` payload.** The only fixture in this repo is
hand-written, and the sibling client (`lib/payat-go/client.ts`) rejects string amounts while
`lib/payat/read.ts` accepts them — the two already disagree about the wire format. Before
flipping either flag, use the admin-gated diagnostic added in the fix wave:
`GET /api/debug/payat-ping?read=<clientAccountNumber>` (requires admin + `PAYAT_DIAG_KEY`).
Run it against an intent whose amount you know and confirm the **unit** — `10000` for an R100
top-up, not `100`. The branch now defers rather than expires on any amount it cannot confirm,
so a mismatch is safe, but it is far better to know before enabling.
...
## Post-implementation addendum (2026-07-30)

### Corrections to this plan, found during implementation

- **The plan's Task 1 test snippet named `createPayatRtp`, which does not exist.** The real
  export is `createPayatPaymentRequest`.
- **`generateClientAccountNumber()` was called inline inside the fetch body and discarded.**
  This made read-back impossible for *any* existing intent and is why Task 1 was a hard
  prerequisite. The two stranded June intents remain unrecoverable in code for this reason —
  they predate the column and must be resolved via the Pay@ portal by hand.
- **The deferral rationale in the Task 5 amendment was partly wrong.** It claimed deferring
  blocks a duplicate top-up and so "stops the provider paying twice". It does not: the
  duplicate-intent guard requires `expiresAt: { gt: now }` and a deferred intent is by
  definition past `expiresAt`. Deferring is justified solely because it keeps the intent
  **creditable**. A double top-up is benign — two payments, two credits, no loss.

### Defects the reviews caught that this plan originally mandated

- **The sweep expired intents on any state it could not confirm.** Only `PENDING` deferred;
  `PAID`-with-unconfirmable-amount, `FAILED` (which covers `PARTIAL_PAYMENT_RECEIVED` and
  `PAYMENT_FEES_ISSUE` — real money paid at a till) and `UNKNOWN` were all expired into a
  permanently uncreditable state. Now an explicit allowlist: expire **only** on `SENT`,
  `EXPIRED`, `CANCELLED`. Everything else defers under the 7-day bound as `indeterminate`.
  A new or unmapped Pay@ state defaults to defer, never expire.
- **Doorbell mode acked and dropped payments the legacy path credits** — it lost the
  `paymentReference` fallback (used when Pay@ sends `reference`/`sourceReference` instead of
  `clientReferenceNumber`) and skipped every pre-migration intent, returning 200 so Pay@ never
  retried. Now falls through to the legacy path for those two cases.
- **The amount guard compared the pre-fee amount**, so read-back would have accepted an
  underpayment the legacy path is tested to reject. Both paths now share
  `resolveExpectedPayatAmountCents()`.
- **A read failure expired the intent.** A throw means we do *not* know whether the provider
  paid — and the most likely trigger is the 403 this very rollout expects before `rtp:read` is
  granted. Now defers under the same 7-day bound.

### Recommended immediate follow-up (not blocking merge)

`lib/payat/reconcile.ts` and the webhook couple by two literal reason strings
(`'intent not found'`, `'no clientAccountNumber'`). The second is pinned by a test; the first
is **not** pinned at its producer, so rewording it would pass CI green and silently restore the
ack-and-drop defect. Export both as constants and import them into the webhook (~5 lines).
