...
- WABA `WABA-ID-EXAMPLE` still resolves to `Kgolaentle Holdings`; account review is `APPROVED`; business verification is `verified`; currency is `USD`.
- Production phone number resolves to `+27 00 000 0000`; Cloud API status is reachable; quality rating is `GREEN`; throughput is `STANDARD`.
...
- WABA `WABA-ID-EXAMPLE` remains `APPROVED`; business verification remains `verified`.
- Phone `+27 00 000 0000` remains Cloud API reachable, `GREEN` quality, `STANDARD` throughput.
...
- If the card was added only to the Meta business profile, ad account, or a general Business Suite payment profile, `131042` may persist. The card must be attached to the WhatsApp Business Account billing asset/payment account for WABA `WABA-ID-EXAMPLE`.

Next required proof:
1. Confirm in Meta Business Suite billing that the payment method is active for the WhatsApp Business Account / WABA asset `WABA-ID-EXAMPLE`, not only the parent business profile.
...
- Direct read of the known payment account ID `PAYMENT-ACCOUNT-EXAMPLE` is blocked by Meta permissions / unsupported object access.
...
- A human must visually confirm the card appears under the WhatsApp Business Account / WABA billing settings for asset `WABA-ID-EXAMPLE`, or authorize one real billable `otp_login` test send as the operational proof.
...
Observed:
- Direct asset billing URL redirected to `business.facebook.com/latest/billing_hub/accounts/details/...asset_id=<WABA_ASSET_ID>...` and showed "Sorry, this content isn't available at the moment".
...
- Billing account page: `payment_account_id=PAYMENT-ACCOUNT-EXAMPLE`
- Business ID: `BUSINESS-ID-EXAMPLE`
- WABA asset ID: `WABA-ID-EXAMPLE`
- WABA selector on Payment methods tab: `Kgolaentle Holdings (WABA-ID-EXAMPLE)`
...
Conclusion:
- The card is not attached to the WhatsApp Business Account billing asset for WABA `WABA-ID-EXAMPLE`.
...
2. Attach the card to `Kgolaentle Holdings (WABA-ID-EXAMPLE)`.
...
Confirmed state:
- URL remained scoped to `business_id=BUSINESS-ID-EXAMPLE`, `asset_id=WABA-ID-EXAMPLE`, `payment_account_id=PAYMENT-ACCOUNT-EXAMPLE`.
- Business portfolio section now shows a MasterCard payment method.
- WhatsApp Business accounts tab remained selected.
- WABA selector shows `Kgolaentle Holdings (WABA-ID-EXAMPLE)`.
- WABA-specific payment method list now shows the same MasterCard as `Default`.

Conclusion:
- The card is now attached to the WhatsApp Business Account billing asset for WABA `WABA-ID-EXAMPLE`.
...
- Meta returned a WhatsApp message ID: `wamid.EXAMPLE-1`.
- No `131042` billing/payment error was returned.

Conclusion:
- Meta billing is now active for WABA `WABA-ID-EXAMPLE`.
...
## 2026-05-13 Provider A login check

User tested Provider A login after card attachment.

Production identity checked:
- Provider: `Provider A`
- Provider status: `ACTIVE`
- Test cohort: `isTestUser=true`
- Login phone: `+27000000002`

Latest OTP delivery attempt:
- Created at `2026-05-13T10:10:57.114Z`
- Template: `otp_login`
- Initial send produced a WhatsApp message ID: `wamid.EXAMPLE-2`
...
Conclusion:
- Provider A's tested OTP did not go through.
...
- Successful verification audit references provider ID `provider-uuid-example`.

Conclusion:
- Provider A OTP delivery and provider login are confirmed working in production.
...
- Payment account ID: PAYMENT-ACCOUNT-EXAMPLE
- WABA asset ID: WABA-ID-EXAMPLE (Kgolaentle Holdings)
- Business ID: BUSINESS-ID-EXAMPLE
...
The phone number in the env var almost certainly resolves to +27 00 000 0000 (PlugAPro), which sits under the Kgolaentle Holdings WABA. That is the same WABA with the missing payment method, which closes the loop on the error.

## Required user action

The AI assistant in this session cannot enter card details, settle balances, or update payment methods. A human must:

1. Open https://business.facebook.com/billing_hub/payment_settings/?asset_id=<WABA_ASSET_ID>&business_id=<BUSINESS_ID>
...
1. Display name "PlugAPro" was REJECTED for +27 00 000 0000. The number remains Connected with High quality, so messages will send under a fallback name (likely the legal entity "Kgolaentle Holdings") until a revised display name is submitted and approved.
2. +27 00 000 0001 (Example Provider Business) is still "In review" / Unverified at Meta. Pending Meta review.
